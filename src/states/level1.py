...
class Level1(settings._State):
    ...
    def prevent_collision_conflict(self, obs1, obs2):
        if obs1 and obs2:
            obs1_distance = self.mario.rect.centerx - obs1.rect.centerx
            if obs1_distance < 0:
                obs1_distance *= -1
            ob2_distance = self.mario.rect.centerx - obs2.rect.centerx
            if ob2_distance < 0:
                ob2_distance *= -1
                
            if obs1_distance < ob2_distance:
                obs2 = False
            else:
                obs1 = False
    ...
    def delete_if_off_screen(self, enemy):
        ...
        elif enemy.state == c.KSLIDE:
            if enemy.rect.x > (self.viewport.right + 500):
                enemy.kill()
    
    def check_flag(self):
        if (self.flag.state == c.BOTTOM_OF_POLE and self.mario.state == c.FLAGPOLE):
