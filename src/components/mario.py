...
class Mario(pg.sprite.Sprite):
    ...
    def standing(self, keys, fire_group):
        ...
        if keys[settings.keybinds['down']]:
            self.crouching = True
        ...
        if keys[settings.keybinds['left']]:
            ...
            self.state = c.WALK
        elif keys[settings.keybinds['right']]:
            self.facing_right = True
            self.get_out_of_crouch()
            self.state = c.WALK
        elif keys[settings.keybinds['jump']]:
            if self.allow_jump:
                if self.big:
        ...
        if not keys[settings.keybinds['down']]:
            self.get_out_of_crouch()
    ...
    def check_to_allow_jump(self, keys):
        if not keys [settings.keybinds['jump']]:
           self.allow_jump = True
            
    def check_to_allow_fireball(self, keys):
        if not keys[settings.keybinds['action']]:
            self.allow_fireball = True
    ...
    def walking(self, keys, fire_group):
        ...
        if keys[settings.keybinds['jump']]:
            if self.allow_jump:
                if self.big:
                    setup.SFX['big_jump'].play()
                else:
